...
Iris::Background::Background ()
    : box_ (Gtk::Orientation::VERTICAL), rightBox_ (), img_ ("/home/alexe/.config/iris/cbg-10.gif"),
      gif_ ("gif", "label", "/home/alexe/.config/iris/cbg-10.gif")
{
  ...
  for (int i = 1000; i < rows.size (); i++)
    this->box_.append (*Gtk::manage (new Iris::Row (i, rows[i].get ("name", "---").asString (),
  ...
  for (int i = 0; i < modules.size (); i++)
    {
      std::cout << "Label: " << modules[i].get ("label", "---").asString ()
                ...
                << "\nScript: " << modules[i].get ("script", "---").asString () << std::endl;
    }
