      ...
      css_provider_ (nullptr)
{
  ...
  stream.open (Iris::Application::home + "/.config/iris/config.json", std::ios::in);
  if (stream.is_open ())
    {
  ...
  struct stat buffer;
  if (stat ((Iris::Application::home + "/.config/iris/style.css").c_str (), &buffer) == 0)
    {
      this->css_provider_ = Gtk::CssProvider::create ();
      this->css_provider_->load_from_path (Iris::Application::home + "/.config/iris/style.css");
...
Iris::Background *
Iris::Application::create_appwindow ()
{
...
void
Iris::Application::on_hide_window (Gtk::Window *_window)
{
  delete _window;
}
