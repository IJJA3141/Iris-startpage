...
std::string Iris::Application::path_home = Glib::getenv("HOME");
      ...
      css_provider_(nullptr)
{
  ...
  stream.open(Iris::Application::path_home + "/.config/iris/config.json", std::ios::in);
  ...
  if (stat((Iris::Application::path_home + "/.config/iris/style.css").c_str(), &buffer) == 0) {
    this->css_provider_ = Gtk::CssProvider::create();
    this->css_provider_->load_from_path(Iris::Application::path_home + "/.config/iris/style.css");

    Gtk::StyleContext::add_provider_for_display(Gdk::Display::get_default(), this->css_provider_,
...
Iris::Background *Iris::Application::create_appwindow()
{
  ...
  if (Iris::Application::config->get("layer_top", true).asBool()) {
    ...
    gtk_layer_set_margin(win->gobj(), GTK_LAYER_SHELL_EDGE_TOP,
  ...
  win->signal_hide().connect(
      sigc::bind(sigc::mem_fun(*this, &Iris::Application::on_hide_window), win));
